...
for link in links:
    ...
    #//---------------------Extracting Data----------------------------------------//
        ...
        tables = new_soup.find_all("div",class_="tablecustom") if new_soup.find_all("div",class_="tablecustom") else None
        
        part = new_soup.find_all("p",class_="tablecolumn")
        manufacturer = part[0].get_text()
        part_no = part[1].get_text()
        # for i in part:
        #     print(i.get_text())

        for table in tables:
            # print((table.find("p",class_="headtable")).get_text())
            rows = table.find_all("div",class_="tablerow")
            for row in rows:
                if row.find_all("div",class_="tablecolumn"):
                    columns = row.find_all("div",class_="tablecolumn")  
                    # print ( f" {(columns[0].find('''span''')).get_text()} : {(columns[1].find('''span''')).get_text()} ")
                else:
                    pass 
